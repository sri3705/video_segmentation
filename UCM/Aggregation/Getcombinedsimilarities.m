...
if ( (Isaffinityrequested(requestedaffinities,'stt')) || (Computationrequestedaffinity(requestedaffinities,'stt',options)) || computeall )
    ...
    timetic=tic;
    fprintf('Mex STT cpp friendly started %f\n',timetic);
    %ADDED BY MEHRAN
    %if exist('/cs/vml3/mkhodaba/cvpr16/Graph_construction/file_features/STT.mat', 'file') ~= 2
        STT=Getshortterm_cpp_friendly(labelledlevelvideo, mapped, flows, graphdepth, multcount, options, theoptiondata, filenames);
    %    save('/cs/vml3/mkhodaba/cvpr16/Graph_construction/file_features/STT.mat', 'STT');
    %else
    %    load('/cs/vml3/mkhodaba/cvpr16/Graph_construction/file_features/STT.mat');
    %end
...
if options.new_features
    ...
    temporaldepth=2;
    timetic=tic;
    [STT_max,STT_mean]=Computetexture(labelledlevelvideo,numberofsuperpixelsperframe,concc,conrr,cim,mapped,thedepth,STT,temporaldepth,printonscreen, options, filenames);
    fprintf('Texture %f\n',toc(timetic));
end
