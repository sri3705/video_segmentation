function [newsimilarities,normpertrack]=Reweightwithhypercliquecardinality(twsimilarities,twlabelledlevelvideo,tmpoptions,twucm2,printonscreen)
...
if ( (~isfield(tmpoptions,'newmethodfrw')) || (isempty(tmpoptions.newmethodfrw)) || (tmpoptions.newmethodfrw) )
    newmethodfrw=true; %New method to contract the similarity matrix, consistent with the pixel or superpixel level relaxed solution
else
    newmethodfrw=false; %Method of Hein using must-link contraints and preserving the cuts
end
...
%Re-weight the coarse similarity matrix according to the equivalent representation
if (newmethodfrw)
    newsimilarities=Reweightequivalently(twsimilarities,normpertrack,maxnotracks);
else
    newsimilarities=Reweightequivalentlyhein(twsimilarities,normpertrack,maxnotracks);
end
