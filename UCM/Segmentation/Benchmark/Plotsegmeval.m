...
if (exist(fullfile(outDir,'eval_bdry_globalthr.txt'),'file'))
    ...
    fprintf(logfile2,'Boundary PR global\n');
    fprintf(logfile2,'   G-ODS: F( R %1.2f, P %1.2f ) = %1.2f   [th = %1.2f]\n',evalRes(2:4),evalRes(1));
    fprintf(logfile2,'   G-OIS: F( R %1.2f, P %1.2f ) = %1.2f\n',evalRes(5:7));
    fprintf(logfile2,'   Area_PR = %1.2f\n',evalRes(8));
...
%     prvals = prvals( (prvals(:,3)>=0.01) ,:); %only those threshold numbers for precision > 0.01
    ...
    fprintf(logfile2,'Volume PR global\n');
    fprintf(logfile2,'   G-ODS: F( R %1.2f, P %1.2f ) = %1.2f   [th = %1.2f]\n',evalRes(2:4),evalRes(1));
    fprintf(logfile2,'   G-OSS: F( R %1.2f, P %1.2f ) = %1.2f\n',evalRes(5:7));
    fprintf(logfile2,'   G-Area_PR = %1.2f\n',evalRes(8));
...
if (exist(fullfile(outDir,'eval_cover.txt'),'file'))
    ...
    fprintf(logfile2,'Region\n');
    fprintf(logfile2,'   GT covering: ODS = %1.2f [th = %1.2f]. OSS = %1.2f. Best = %1.2f\n',evalRes(2),evalRes(1),evalRes(3:4));
...
if (exist(fullfile(outDir,'eval_RI_VOI.txt'),'file'))
    ...
    fprintf(logfile2,'Region\n');
    fprintf(logfile2,'   Rand Index: ODS = %1.2f [th = %1.2f]. OSS = %1.2f.\n',evalRes(2),evalRes(1),evalRes(3));
    fprintf(logfile2,'   Var. Info.: ODS = %1.2f [th = %1.2f]. OSS = %1.2f.\n',evalRes(5),evalRes(4),evalRes(6));
...
if ( (exist(fullfile(outDir,'eval_regpr_avgthr.txt'),'file')) && (exist(fullfile(outDir,'eval_nclustersstats_thr.txt'),'file')) && (true) )
    ...
    if (true)
        ...
        fprintf(logfile2,'Length stats with Volume PR global\n');
        fprintf(logfile2,'   Length best F [th = %1.2f] G-ODS: mean %1.2f, std %1.2f \n',evalRes(1), lengthvals(idx(1),2), lengthvals(idx(1),3) );
    ...
    if (true)
        ...
        fprintf(logfile2,'Ncluster stats with Volume PR global\n');
        fprintf(logfile2,'   Ncluster best F [th = %1.2f] G-ODS: avg ncluster %1.2f \n',evalRes(1), nclustervals(idx(1),2) );
